...
tcp_server_handle_s tcpserver_s = {0, 0, NULL, NULL, NULL,NULL, 0, {0}};
...
u8 tcp_server_init(void)
{
	...
	if(tcppcbnew)			//创建成功
	{
		tcpserver_sp->localpcb = tcppcbnew;
...
void tcp_server_prcs(void)
{
	switch(tcpserver_sp->server_state)
	{
		case ES_TCPSERVER_NONE: //第一次初始化
			...
			{
				tcp_server_connection_close(tcpserver_sp->localpcb,0);//关闭TCP Server连接
		...
		case ES_TCPSERVER_ACCEPTED:
			...
			{
				printf("recv: %s\n", tcpserver_sp->recvbuf);
				tcpserver_sp->recv_state = 0;
				//zkrt_todo: 解析数据处理
				//test yanly
				tcp_server_send(tcpserver_sp->pcb, tcpserver_sp, tcpserver_sp->recvbuf, strlen((char *)tcpserver_sp->recvbuf));
			}
			break;
